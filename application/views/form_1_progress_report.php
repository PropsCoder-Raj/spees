            ...
            <div class="container">
                <div class="row">
                    <div class="col-lg-12">
                            ...
                            <div class="card" style="width: 100%;">
                                <div class="card-body">
                                    <table class="table">
                                        ...
                                        <tbody id="tbody">
                                            <?php $i = 0; foreach($progressReportData as $item): ?>
                                            <tr>
                                                <td><input type="text" class="form-control"
                                                        id="baseLine_update_<?php echo $i; ?>" name="baseLine[]"
                                                        placeholder="Enter Baseline"
                                                        value="<?php echo $item->BaseLine;?>"><input type="text"
                                                        style="display: none;" name="formId[]"
                                                        value="<?php echo $item->Id; ?>"></td>
                                                <td><input type="text" class="form-control"
                                                        id="goal_update_<?php echo $i; ?>" name="goal[]"
                                                        placeholder="Enter Goal" value="<?php echo $item->Goal; ?>">
                                                </td>
                                                <td><input type="text" class="form-control"
                                                        id="progress_update_<?php echo $i; ?>" name="progress[]"
                                                        placeholder="Enter Progress"
                                                        value="<?php echo $item->Progress; ?>"></td>
                                                <td></td>
                                            </tr>
                                            <?php $i++; endforeach; ?>
                                        </tbody>
                        ...
                        <?php if($action == 1){ ?>
                            <?php if($edit === true): ?>
                            <button type="submit" class="btn btn-primary float-right" name="edit_progress_report"
                                id="edit_progress_report">Submit</button>
                            <?php else: ?>
                            <button type="submit" class="btn btn-primary float-right" name="add_progress_report"
                                id="add_progress_report">Submit</button>
                            <?php endif; ?>
                        <?php } ?>    
                    </div>
                </div>
...
function addItem() {
    ...
    var html = `
        <tr>
            <td><input type="text" class="form-control" id="baseLine` + items +
        `" name="baseLine[]" placeholder="Enter Baseline"><input type="text" style="display: none;" name="formId[]" value="empId` +
        items + `"></td>
            <td><input type="text" class="form-control" id="goal` + items + `" name="goal[]" placeholder="Enter Goal"></td>
            <td><input type="text" class="form-control" id="progress` + items + `" name="progress[]" placeholder="Enter Progress"></td>
